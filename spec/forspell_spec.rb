...
RSpec.describe Forspell do
  let(:checker) { described_class.new(paths: '') } # with default en-Us dictionary
  ...
  describe '#check_spelling' do
    ...
    describe 'checking readme-s errors' do
      ...
      let(:checker) { described_class.new(paths: filepath, no_output: true) }
    ...
    context 'with fixtured grouped examples' do
      subject { described_class.new(paths: '').check_spelling(input) }
